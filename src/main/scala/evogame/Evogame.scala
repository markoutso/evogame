...
import evogame.evolution.types.{Conway, Grid}
...
//  val grid = Grid(40, Grid.range(40).map(p => if (positions.contains(p.swap)) true else false))
  ...
  (1 to 10) foreach { _ =>
    println(c.state.cells.count(_ == true))
    c = c.grow
  }
...
object Evogame extends JSApp {
  ...
  def main() = {
    ...
    val grid = Grid(80, Grid.range(80).map(p => if (positions.contains(p.swap)) true else false))
